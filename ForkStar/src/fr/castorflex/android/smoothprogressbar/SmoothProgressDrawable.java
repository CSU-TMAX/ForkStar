...
public class SmoothProgressDrawable extends Drawable implements Animatable {
	...
	private SmoothProgressDrawable(Interpolator interpolator,
			...
			float strokeWidth, float speed, float progressiveStartSpeed,
			float progressiveStopSpeed, boolean reversed, boolean mirrorMode,
			Callbacks callbacks, boolean progressiveStartActivated,
			Drawable backgroundDrawable, boolean useGradients) {
		mRunning = false;
	...
	public void setProgressiveStartSpeed(float speed) {
		if (speed < 0)
			throw new IllegalArgumentException(
					"SpeedProgressiveStart must be >= 0");
	...
	public void setProgressiveStopSpeed(float speed) {
		if (speed < 0)
			throw new IllegalArgumentException(
					"SpeedProgressiveStop must be >= 0");
	...
	private void drawGradient(Canvas canvas) {
		...
		for (int i = 0; i < mSectionsCount; ++i) {
			...
			float position = mInterpolator.getInterpolation(i * xSectionWidth
					+ mCurrentOffset);
		...
		float left = mReversed ? (mMirrorMode ? Math.abs(mBounds.left
				- mBounds.right) / 2 : mBounds.left) : mBounds.left;
		float right = mMirrorMode ? (mReversed ? mBounds.left : Math
				.abs(mBounds.left - mBounds.right) / 2) : mBounds.right;
		...
		LinearGradient linearGradient = new LinearGradient(left, top, right,
				bottom, mLinearGradientColors, mLinearGradientPositions,
	...
	private void drawStrokes(Canvas canvas) {
		...
		for (int i = 0; i <= mCurrentSections; ++i) {
			...
			if (end > prevValue && i >= mStartSection) {
				float xFinishingOffset = mInterpolator.getInterpolation(Math
						.min(mFinishingOffset, 1f));
				startX = Math.max(xFinishingOffset * width,
						Math.min(boundsWidth, prevValue));
				endX = Math.min(boundsWidth, end);
				drawLine(canvas, boundsWidth, startX, centerY, endX, centerY,
						currentIndexColor);
	...
	private void drawBackgroundIfNeeded(Canvas canvas, float firstX, float lastX) {
		...
		fBackgroundRect.right = mMirrorMode ? canvas.getWidth() / 2 : canvas
				.getWidth();
		...
		if (firstX > 0) {
			if (mMirrorMode) {
				...
				if (mReversed) {
					...
					drawBackground(canvas, canvas.getWidth() / 2 - firstX,
							canvas.getWidth() / 2);
					canvas.scale(-1, 1);
					drawBackground(canvas, canvas.getWidth() / 2 - firstX,
							canvas.getWidth() / 2);
	...
	public static class Builder {
		...
		public SmoothProgressDrawable build() {
			if (mGenerateBackgroundUsingColors) {
				mBackgroundDrawableWhenHidden = SmoothProgressBarUtils
						.generateDrawableWithColors(mColors, mStrokeWidth);
			}
			SmoothProgressDrawable ret = new SmoothProgressDrawable(
					mInterpolator, mSectionsCount, mStrokeSeparatorLength,
					mColors, mStrokeWidth, mSpeed, mProgressiveStartSpeed,
					mProgressiveStopSpeed, mReversed, mMirrorMode,
					mOnProgressiveStopEndedListener,
					mProgressiveStartActivated, mBackgroundDrawableWhenHidden,
					mGradients);
			return ret;
		}
		...
		private void initValues(Context context) {
			...
			mProgressiveStartActivated = res
					.getBoolean(R.bool.spb_default_progressiveStart_activated);
			mGradients = false;
		}
		...
		public Builder progressiveStartSpeed(float progressiveStartSpeed) {
			if (progressiveStartSpeed < 0)
				throw new IllegalArgumentException(
						"progressiveStartSpeed must be >= 0");
			mProgressiveStartSpeed = progressiveStartSpeed;
			return this;
		}
		...
		public Builder progressiveStopSpeed(float progressiveStopSpeed) {
			if (progressiveStopSpeed < 0)
				throw new IllegalArgumentException(
						"progressiveStopSpeed must be >= 0");
			mProgressiveStopSpeed = progressiveStopSpeed;
			return this;
		}
